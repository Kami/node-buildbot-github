...
BuildBot.prototype.sendChanges = function(pullId, revision, user, project,
  ...
  var user, email, nickname, who;

  if (user && user.name) {
      name = user.name.replace(/[^a-zA-Z0-9-._~\s]/g, ''); // Buildbot doesn't like unicode
      nickname = user.login;
      email = user.email;
  }
  else {
      log.info('Missing user name, using unknown', {'user': user});
